...
for pointing in {0..184}; do
    ...
    de_file="imap_ultra_l1b_45sensor-de_20000101-repoint${pointing_padded}_v100.cdf"
    rates_file="imap_ultra_l1a_45sensor-rates_20000101-repoint${pointing_padded}_v100.cdf"

    echo "Processing pointing ${pointing}"
    echo "Pointing: ${pointing}"
    echo "Date result: $(date -j -v+${pointing}d -f "%Y%m%d" "20250416" +"%Y%m%d")"

    # If that works, then use it in the command:
    start_date=$(date -j -v+${pointing}d -f "%Y%m%d" "20250416" +"%Y%m%d")
    ...
    imap_cli --instrument ultra \
        ...
        --version v001 \
        --dependency '[
            {"type": "science","files": ["imap_ultra_l1b_45sensor-goodtimes_20260926_v003.cdf"]},
            {"type": "science","files": ["'"$de_file"'"]},
            {"type": "science","files": ["'"$rates_file"'"]},
            {"type": "science","files": ["imap_ultra_l1a_45sensor-params_20260926_v009.cdf"]},
            ...
            {"type": "ancillary","files": ["imap_ultra_l1b-sensor-gf-noblades_20250101_v000.csv"]},
