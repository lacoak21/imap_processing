...
def test_l1b_cdf_filenames(l1b_dataset: xr.Dataset):
    """Tests that the ``process_idex_l1b`` function generates datasets
...
def test_unpack_instrument_settings():
    ...
    test_cdf_defs_df = pd.DataFrame(
        {
            ...
            "packetName": ["idx__test_var01", "idx__test_var01"],
            "starting_bit": [0, 2],
    ...
    unpacked_dict = unpack_instrument_settings(test_ds, test_cdf_defs_df)
