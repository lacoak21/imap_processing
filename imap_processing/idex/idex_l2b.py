...
.. code-block:: python
    ...
    l2b_data = idex_l2b(l2a_data, [evt_data, evt_data2])
    write_cdf(l2b_data)
"""
...
def idex_l2b(l2a_dataset: xr.Dataset, evt_datasets: list[xr.Dataset]) -> xr.Dataset:
    ...
    l2b_dataset["sci_acquisition_events"] = xr.DataArray(
        name="sci_acquisition_events",
        data=np.arange(len(evt_logs)),
        dims="sci_acquisition_events",
        attrs=idex_attrs.get_variable_attributes(
            "sci_acquisition_events", check_schema=False
        ),
    )
...
def get_science_acquire_timestamps(
    ...
    return np.asarray(event_logs), np.asarray(event_timestamps)
